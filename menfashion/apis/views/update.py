...
from varieties.models import SubCategory, Currency, Country, City, Brand, SubType, Type, Size, Colors, Condition, PaymentMethod
...
def update_mens_fashion_ad(request, ad_id):
    data = request.data
    ...
    try:
        ad = Fashion.objects.get(pk=ad_id)
    ...
    try:
        ...
        if mechanism and ad.mechanism.name != mechanism:
            try:
                mechanism_model = Mechanism.objects.get(name=mechanism)
            except Mechanism.DoesNotExist:
                return Response({
                    "message": "this mechanism doesn't exists"
                }, status=status.HTTP_404_NOT_FOUND)
            ad.mechanism = mechanism_model
            ad.save()
            is_updated = True
