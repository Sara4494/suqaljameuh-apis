...
from jobs.models import JobOffer, ExperienceLevel, ContractType, Grade, JobTitle, JobSector
...
def update_offer(request, offer_id):
    ...
    try:
        ad = JobOffer.objects.get(pk=offer_id)
    ...
    try:
        is_updated = False
        if job_title and ad.job_title.name != job_title:
            try:
                job_title_model = JobTitle.objects.get(name=job_title)
            except JobTitle.DoesNotExist:
                return Response({
                    "message": "Cannot find job title with this name"
                }, status=status.HTTP_404_NOT_FOUND)
            ad.job_title = job_title_model
            is_updated = True

        if job_sector and ad.job_sector.name != job_sector:
            try:
                job_sector_model = JobSector.objects.get(name=job_sector)
            except JobSector.DoesNotExist:
                return Response({
                    "message": "Cannot find job sector with this name"
                }, status=status.HTTP_404_NOT_FOUND)
            ad.job_sector = job_sector_model
            is_updated = True
