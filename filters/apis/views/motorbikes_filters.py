...
def motorbikes_filters(data):
    ads = MotorBikes.objects.all()
    ...
    if is_valid_query(kilo_meter):
        ads = ads.filter(kilo_meter__name=kilo_meter)

    if is_valid_query(capacity):
        ads = ads.filter(capacity__name=capacity)

    if is_valid_query(brand):
        ads = ads.filter(brand__name=brand)

    if is_valid_query(model):
...
def basic_filter(request):
    try:
        ads = motorbikes_filters(request.data)
