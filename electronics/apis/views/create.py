...
from varieties.models import SubCategory, Currency, Country, City, Brand, Type, Capacity, Condition, PaymentMethod
...
def create_ad(user, sub_category, title, description, country_model, city_model, currency_model, payment_method_model, price, full_name=None, phone_number=None, lat=None, lat_delta=None, long=None, long_delta=None, is_delivery=False, brand_model=None, type_model=None, condition_model=None, capacity_model=None):
    ad = Electronics.objects.create(
        user=user,
...
def publish_ad(request):
    ...
    if capacity:
        try:
            capacity_model = Capacity.objects.get(name=brand)
        except Capacity.DoesNotExist:
            return Response({
                "message": "There's no capacity with this name"
            }, status=status.HTTP_404_NOT_FOUND)
