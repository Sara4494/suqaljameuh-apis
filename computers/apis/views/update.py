...
def update_ad(request, ad_id):
    ...
    try:
        ...
        if screen_size and ad.screen_size.name != screen_size:
            try:
                screen_size_model = ScreenSize.objects.get(name=screen_size)
            except ScreenSize.DoesNotExist:
                return Response({
                    "message": "this screen size doesn't exists"
                }, status=status.HTTP_404_NOT_FOUND)
            ad.screen_size = screen_size_model
            ad.save()
            is_updated = True
        ...
        if os and ad.os.name != os:
            try:
                os_model = OS.objects.get(name=os)
            except OS.DoesNotExist:
                return Response({
                    "message": "this operating system doesn't exists"
                }, status=status.HTTP_404_NOT_FOUND)
            ad.os = os_model
            ad.save()
            is_updated = True

        if storage and ad.storage.name != storage:
            try:
                storage_model = Storage.objects.get(name=storage)
            except Storage.DoesNotExist:
                return Response({
                    "message": "this storage doesn't exists"
                }, status=status.HTTP_404_NOT_FOUND)
            ad.storage = storage_model
            ad.save()
            is_updated = True

        if memory and ad.memory.name != memory:
            try:
                memory_model = Memory.objects.get(name=memory)
            except Memory.DoesNotExist:
                return Response({
                    "message": "this memory doesn't exists"
                }, status=status.HTTP_404_NOT_FOUND)
            ad.memory = memory_model
            ad.save()
            is_updated = True
