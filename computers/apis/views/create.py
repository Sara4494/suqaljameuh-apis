...
from varieties.models import SubCategory, Currency, Country, City, Brand, Model, Condition, PaymentMethod, OS, Storage, Memory, Colors
from computers.models import ScreenSize
...
def create_ad(user, sub_category, title, description, country_model, city_model, currency_model, payment_method_model, price, full_name=None, phone_number=None, lat=None, lat_delta=None, long=None, long_delta=None, is_delivery=False, brand_model=None, model_table=None, os_model=None, storage_model=None, memory_model=None, condition_model=None, screen_size_model=None, color_model=None):
    ad = Computer.objects.create(
        user=user,
...
def publish_ad(request):
    ...
    if screen_size:
        try:
            screen_size_model = ScreenSize.objects.get(name=screen_size)
        except ScreenSize.DoesNotExist:
            return Response({
                "message": "There's no screen size with this name"
            }, status=status.HTTP_404_NOT_FOUND)
    ...
    if os:
        try:
            os_model = OS.objects.get(name=os)
        except OS.DoesNotExist:
            return Response({
                "message": "There's no os with this name"
            }, status=status.HTTP_404_NOT_FOUND)

    if storage:
        try:
            storage_model = Storage.objects.get(name=storage)
        except Storage.DoesNotExist:
            return Response({
                "message": "There's no storage with this name"
            }, status=status.HTTP_404_NOT_FOUND)

    if memory:
        try:
            memory_model = Memory.objects.get(name=memory)
        except Memory.DoesNotExist:
            return Response({
                "message": "There's no memory with this name"
            }, status=status.HTTP_404_NOT_FOUND)
    ...
    if user_membership != None:
        ...
        try:
            ad = create_ad(
                ...
                screen_size_model=screen_size_model,
