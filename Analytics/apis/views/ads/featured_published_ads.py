...
def GetWeklyFuturedPublishedAd (request) : 
    try : 
        
        to = datetime.datetime.now() - datetime.timedelta(weeks=1)
        from_ = to - datetime.timedelta(weeks=1)
        ads_in_this_week = Ad.objects.filter(ad_type__in=('Featured','Very Featured') , published_at__gte = to )
        
        ads_for_last_weeks = Ad.objects.filter(ad_type__in=('Featured','Very Featured') , published_at__gte = from_, published_at__lte = to )
