from rest_framework.decorators import api_view, permission_classes
from rest_framework.response import Response
from apartments.models import Interface
from apartments.apis.serializers import InterfaceSerializer
from rest_framework import permissions, status
from django.utils.translation import gettext as _

@api_view(['DELETE'])
@permission_classes([permissions.IsAdminUser])
def delete_interface(request, pk):
    try:
        interface = Interface.objects.get(pk=pk)
        interface.delete()
        return Response({'message': _('Interface deleted successfully.')}, status=status.HTTP_200_OK)
    except Interface.DoesNotExist:
        return Response({'error': _('Interface object not found.')}, status=status.HTTP_404_NOT_FOUND)
    except Exception as e:
        return Response({'error': str(e), 'message': _('Internal server error occurred.')}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)

 