...
def update_site(request, pk):
    try:
        ...
        return Response({'error': _('Site does not exist')}, status=status.HTTP_404_NOT_FOUND)

    try:
        name = request.data.get('name')
        if name and Site.objects.filter(name=name).exclude(pk=pk).exists():
            return Response({'error': _('This name is already registered at the Site.')}, status=status.HTTP_400_BAD_REQUEST)
        serializer = SiteSerializer(site, data=request.data)
        if serializer.is_valid():
            serializer.save()
            return Response({'success': _('Site updated successfully')}, status=status.HTTP_200_OK)
